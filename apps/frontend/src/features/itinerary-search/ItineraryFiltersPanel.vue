...
<template>
  <aside class="filters" aria-label="Itinerary filters">
    ...
    <label>Booking type<select :value="query.bookingType ?? ''" @change="set('bookingType', ($event.target as HTMLSelectElement).value || undefined)"><option value="">All booking types</option><option value="separateTickets">Separate tickets</option><option value="bundledFare">Bundled fare</option></select></label>
