...
const loadPath = async (codes: string[]) => {
  ...
  error.value = ''
  try {
    const responses: ExploreRoutesResponse[] = []
    for (const code of codes) responses.push(await getExploreRoutes(code, requestController.signal))
...
watch(() => picker.airports.value[0]?.code, code => {
  ...
  if (code) { void loadPath([code]); void router.replace({ path: '/explore', query: { path: code } }) }
...
watch(() => route.query.path, () => {
  ...
  if (codes.length && codes.join(',') !== pathCodes.value.join(',') && codes.join(',') !== requestedPath) void loadPath(codes)
}, { immediate: true })
...
const searchFares = () => {
  ...
  void router.push({ path: '/search', query: { origins: committedPath.value[0].code, destinations: selectedDestination.value.code, prefill: 'true' } })
}
const commitSelection = () => {
  ...
  selectedDestination.value = null
  hoveredDestination.value = null
  void router.push({ path: '/explore', query: { path: codes.join(',') } })
  void loadPath(codes)
}
const truncatePath = (index: number) => void router.push({ path: '/explore', query: { path: pathCodes.value.slice(0, index + 1).join(',') } })
const continueInBuilder = () => {
  if (!canContinueInBuilder.value) return
  void router.push({ path: '/multi-destination', query: { mode: 'ordered', route: builderPath.value.map(airport => airport.code).join(','), prefill: 'true' } })
}
...
<template>
  <main id="main-content" class="explore-page" tabindex="-1">
    <header class="explore-heading">
      ...
      <p>Choose one airport to see the direct destinations observed in its current FlightAPI schedule window. Scheduled service does not guarantee that a fare is available for your dates, and this rolling window does not include every seasonal route.</p>
    </header>
    ...
    <section v-if="routes" :key="routes.origin.code" class="explore-results" :class="{ 'explore-results--updating': loading }" :aria-busy="loading" aria-labelledby="routes-heading">
      <div class="network-heading">
        <div><p class="eyebrow">Current network</p><h2 id="routes-heading">{{ routes.origin.city }} connects directly to {{ routes.destinations.length }} destination{{ routes.destinations.length === 1 ? '' : 's' }}</h2></div>
      ...
      <p v-if="!routes.isComplete" class="network-note network-note--warning">The provider reported more schedule pages than this deployment permits, so this map is partial.</p>
      <p class="network-note">Schedule window: {{ routes.observedFrom }} through {{ routes.observedTo }}. Routes may include codeshares, but each destination appears once.</p>
