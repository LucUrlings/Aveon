...
const airportName = (airport: ExploreAirport) => airport.city || airport.name
const routeLabel = computed(() => previewPath.value.map(airportName).join(' → '))
...
const clearRoute = () => {
  controller?.abort()
  controller = null
  suppressedPickerCode = ''
  requestedPath = ''
  loadedPath = ''
  picker.input.value = ''
  picker.airports.value = []
  routes.value = null
  committedPath.value = []
  selectedDestination.value = null
  hoveredDestination.value = null
  ...
  loading.value = false
  error.value = ''
  void router.replace({ path: '/explore' })
}
  ...
  <main id="main-content" class="explore-page" tabindex="-1">
    ...
    <Transition name="route-tray">
      <nav v-if="committedPath.length" class="route-tray" aria-label="Explored route">
        ...
        <ol><li v-for="(airport, index) in committedPath" :key="airport.code"><button type="button" :aria-label="`Return to ${airport.city}`" :title="`${airport.name} (${airport.code})`" @click="truncatePath(index)">{{ airportName(airport) }}</button><b v-if="index < committedPath.length - 1" aria-hidden="true">→</b></li></ol>
        ...
        <button type="button" class="tray-clear" @click="clearRoute">Clear route</button>
      </nav>
    </Transition>
        ...
        <div class="globe-column">
          ...
          <article v-if="selectedDestination" class="route-selection" aria-live="polite">
            ...
            <div class="selection-actions">
              ...
              <button type="button" :disabled="!canCommitSelection" @click="commitSelection">{{ committedPath.length === 1 ? `Explore onward from ${airportName(selectedDestination)}` : `Keep exploring from ${airportName(selectedDestination)}` }}</button>
          ...
          <RouteGlobe :routes="routes" :committed-path="committedPath" :selected-destination="selectedDestination" :hovered-destination="hoveredDestination" :auto-rotate="false" @select="selectDestination" />
        </div>
        <aside class="destination-browser" aria-labelledby="destination-list-heading">
          ...
          <ul>
            <TransitionGroup name="destination">
              <li v-for="destination in filteredDestinations" :key="destination.code"><button type="button" :disabled="pathCodes.includes(destination.code)" :aria-pressed="selectedDestination?.code === destination.code" @click="selectDestination(destination)" @focus="hoveredDestination = destination" @blur="hoveredDestination = null" @keydown.enter.prevent="selectDestination(destination)" @keydown.space.prevent="selectDestination(destination)"><span><strong>{{ destination.city }}</strong><small>{{ destination.name }}</small></span><b>{{ destination.code }}</b></button></li>
            </TransitionGroup>
          </ul>
...
.explore-page { width: min(var(--content-max), calc(100% - 48px)); }
