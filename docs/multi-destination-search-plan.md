...
This document is the source of truth for Aveon's pivot from flight comparison into an advanced travel search platform. Every implementation task and pull request for this initiative must reference a milestone and acceptance criterion from this plan. If a product decision changes, update this document before changing the implementation.
...
The primary search page exposes:

- `One way`
- `Return`
- `Multi-destination`

Multi-destination opens a dedicated route with two tabs:
...
- [x] Route every currently implemented FlightAPI operation—airport lookup/autocomplete, one-way, and round-trip—through one singleton FlightAPI gate.
...
- A mixed test workload containing airport lookup, one-way, and round-trip operations never exceeds five combined live requests.
